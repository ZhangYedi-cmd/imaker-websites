...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
  ...
  @Override
  public LoginResponse captchaLogin(String captcha) {
    if (StringUtils.isAnyBlank(captcha)) {
      throw new BusinessException(ErrorCodeEnum.NULL_PARAMS);
    }
    QueryWrapper<User> wrapper = new QueryWrapper<>();
    wrapper.eq("captcha", captcha);
    User user = this.getOne(wrapper);
    if (user == null) {
      throw new BusinessException(ErrorCodeEnum.CAPTCHA_ERROR);
    }
    long captchaTime = user.getCaptchaTime().getTime();
    if (new Date().getTime() - captchaTime > 600000) {
      throw new BusinessException(ErrorCodeEnum.CAPTCHA_HAS_EXPIRE);
    }
    // 微信用户，直接拿openid做token
    String openId = user.getOpenId();
    String token = user.getId() + "#" + openId;
    redisService.setValue(String.valueOf(user.getId()), token);
    LoginResponse loginResponse = new LoginResponse();
    loginResponse.setToken(token);
    loginResponse.setSafeUser(user);
